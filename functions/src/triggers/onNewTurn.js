    ...
    .onDocumentCreated('turns/{id}', async (event) => {
        ...
        try{
            ...
            const oneMonthAgoTurns = await db.collection('turns')
                .where('idUser', '==', idUser)
                .where('initialTimestamp', '>=', oneMonthAgo)
                .get();
            ...
            oneMonthAgoTurns.docs.forEach(doc =>{
                const turn = doc.data();
                const placeInfo = placeFrequency.get(turn.idPlace) || {count:0, lastVisit: turn.initialTimestamp}
                
                placeFrequency.set(turn.idPlace, {
                    count: placeInfo.count+1, 
                    lastVisit: turn.initialTimestamp.toDate() > placeInfo.lastVisit.toDate()
                        ? turn.initialTimestamp
                        : placeInfo.lastVisit
                });
            });
